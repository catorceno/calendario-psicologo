...
import { sendConfirmationEmail, sendRescheduledEmail } from "../scripts/email-notifier.js";
...
it("sendRescheduledEmail_correoValido_Exitoso", async () => {
  // Arrange
  const fakeFetch = createFakeFetch();
  const event = {
    title: "Ines Orellana",
  ...
  // Act
  await sendRescheduledEmail(event, { fetch: fakeFetch });
